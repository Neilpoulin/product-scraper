...
exports.helloWorld = (req, res) => {
    ...
    processUrl(url).then(response => {
        return res.status(200).send('successfully processed url');
    }).catch(error => {
...
function processUrl(url) {
    if (url.indexOf('www.etsy.com') !== -1){
        ...
        return getEtsyListing(listingId, url)
    } else {
...
function getEtsyListing(listingId, url)
{
    return axios.get(`${etsyDomain}${listingIdPath.replace(':listingId', listingId)}?api_key=${apiKey}&includes=MainImage`)
        .then(({data}) =>  data).then(data => {
            return processListings(data.results, url);
        })
}
